...
function Icon({ type, size = 27 }: { type: "home" | "calendar" | "history" | "profile" | "check" | "clock"; size?: number }) {
  ...
  if (type === "check") return <svg {...p}><path d="m5 12 4 4 10-10"/></svg>
  if (type === "clock") return <svg {...p}><circle cx="12" cy="12" r="8.7"/><path d="M12 7v5l3 2"/></svg>
...
function formatDate(key: string) { return new Date(`${key}T00:00:00`).toLocaleDateString("en-US", { month: "short", day: "numeric" }) }
...
export default function HistoryPage() {
  ...
  const doses = useMemo(() => {
    ...
    for (let i = 0; i < 7; i++) {
      const date = keyFor(daysBack(i))
      for (const [medId, dateRecords] of Object.entries(records)) {
        const medication = medicationMap.get(medId)
        if (!medication || medication.active === false) continue
        if (medFilter !== "all" && medId !== medFilter) continue
        const times = dateRecords?.[date] || {}
        for (const [time, value] of Object.entries(times)) {
          const status = value?.status
          if (status) out.push({ medication, date, time, status })
        }
      }
    }
    return out.sort((a, b) => `${b.date} ${b.time}`.localeCompare(`${a.date} ${a.time}`))
  }, [medications, records, medFilter])
  ...
  const week = doses.reduce<Record<string, { taken: number; total: number }>>((acc, d) => { if (!acc[d.date]) acc[d.date] = { taken: 0, total: 0 }; acc[d.date].total++; if (d.status === "taken") acc[d.date].taken++; return acc }, {})
...
function BottomNav({ router }: { router: ReturnType<typeof useRouter> }) {
  ...
  return <nav style={{ position: "fixed", left: 0, right: 0, bottom: 0, height: 76, background: "rgba(255,255,255,.98)", borderTop: "1px solid #dce7e3", display: "flex", justifyContent: "center", zIndex: 20 }}><div style={{ width: "100%", maxWidth: 760, display: "grid", gridTemplateColumns: "repeat(4,1fr)" }}>{items.map(x => <button key={x.label} onClick={() => router.push(x.path)} style={{ border: 0, background: "transparent", color: x.label === "History" ? "#3eaa7d" : "#657786", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 3, fontWeight: x.label === "History" ? 800 : 500 }}><Icon type={x.icon} size={27}/><span style={{ fontSize: 13 }}>{x.label}</span></button>)}</div></nav>
}
