...
export default function SchedulePage() {
  ...
  return <main style={{ minHeight: "100vh", background: "#edf5f2", color: "#142234", paddingBottom: 82, fontFamily: "Arial,Helvetica,sans-serif" }}>
    ...
    <div className="schedule-wrap" style={{ maxWidth: 760, margin: "0 auto", padding: "30px 22px" }}>
      ...
      {active.length === 0 ? <div style={{ background: "white", borderRadius: 25, padding: 28, textAlign: "center", border: "1px solid #dce7e3" }}><div style={{ fontSize: 20, fontWeight: 800 }}>No medications scheduled</div><p style={{ color: "#71808e" }}>Add a medication to create your schedule.</p></div> : <div style={{ display: "grid", gap: 12 }}>{active.map(({ medication, time }) => { const status = statusTree[medication.id]?.[selectedKey]?.[time]?.status ?? "pending"; return <article key={medication.id} style={{ background: "white", border: "1px solid #dce7e3", borderRadius: 25, padding: "16px 18px", display: "flex", alignItems: "center", gap: 14 }}><div style={{ width: 55, height: 55, borderRadius: "50%", background: "#e3eef9", color: "#4d79a7", display: "grid", placeItems: "center" }}><Icon type="pill" size={27}/></div><div style={{ flex: 1 }}><div style={{ fontSize: 19, fontWeight: 800 }}>{medication.name}</div><div style={{ color: "#71808e", marginTop: 3 }}>{medication.dosage} {medication.unit} · {time}</div></div><div style={{ color: status === "taken" ? "#3c9c77" : status === "missed" ? "#ce555d" : "#657786", background: status === "taken" ? "#def2e8" : status === "missed" ? "#fde7e7" : "#eef1f2", padding: "9px 14px", borderRadius: 22, fontWeight: 800 }}>{status[0].toUpperCase() + status.slice(1)}</div></article> })}</div>}
    </div>
